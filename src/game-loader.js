(async () => {
  const baseUrl = 'src/game.js?base=15';
  const response = await fetch(baseUrl, { cache: 'no-store' });
  let code = await response.text();

  code = code.replace(
    "  const W = canvas.width;\n  const H = canvas.height;\n  ctx.imageSmoothingEnabled = false;",
    "  const LOGICAL_W = 450;\n  const LOGICAL_H = 800;\n  const RENDER_SCALE = Math.min(Math.max(window.devicePixelRatio || 2, 2), 3);\n  canvas.width = Math.round(LOGICAL_W * RENDER_SCALE);\n  canvas.height = Math.round(LOGICAL_H * RENDER_SCALE);\n  canvas.style.width = 'min(100vw, 56.25svh)';\n  canvas.style.height = 'min(100svh, 177.78vw)';\n  const W = LOGICAL_W;\n  const H = LOGICAL_H;\n  ctx.setTransform(RENDER_SCALE, 0, 0, RENDER_SCALE, 0, 0);\n  ctx.imageSmoothingEnabled = true;"
  );

  code = code.replace(
    /fetch\('assets\/menu-bg\.txt\?v=15', \{ cache: 'no-store' \}\)[\s\S]*?\.catch\(\(\) => \{ menuBgReady = false; \}\);/,
    "menuBg.onload = () => { menuBgReady = true; };\n  menuBg.src = 'assets/70282CA7-7E9E-472D-A931-2D9E3FAF72AF.jpg?v=29';"
  );

  code = code.replace(
    "  ctx.imageSmoothingEnabled = true;\n\n  const CAR_SCREEN_Y",
    "  ctx.imageSmoothingEnabled = true;\n\n  const GREEN_FIELD_LOOP_END = 13.0;\n  const gameAudio = { green: null, dry: null, brakes: null, current: null, active: false, field: '' };\n\n  function makeGameAudio(src, loop, volume) {\n    const audio = new Audio(src);\n    audio.loop = loop;\n    audio.volume = volume;\n    audio.preload = 'auto';\n    return audio;\n  }\n\n  function initGameAudio() {\n    if (gameAudio.green) return;\n    gameAudio.green = makeGameAudio('assets/Green%20Field.mp3?v=29', false, 0.54);\n    gameAudio.dry = makeGameAudio('assets/Dry%20Field.mp3?v=29', true, 0.54);\n    gameAudio.brakes = makeGameAudio('assets/Brakes.mp3?v=29', true, 0.68);\n  }\n\n  function playAudioTrack(track) {\n    if (!track) return;\n    const p = track.play();\n    if (p && typeof p.catch === 'function') p.catch(() => {});\n  }\n\n  function pauseAudioTrack(track, reset = false) {\n    if (!track) return;\n    track.pause();\n    if (reset) {\n      try { track.currentTime = 0; } catch (_) {}\n    }\n  }\n\n  function switchFieldAudio(field) {\n    if (!gameAudio.active || gameAudio.field === field) return;\n    const next = field === 'dry' ? gameAudio.dry : gameAudio.green;\n    if (gameAudio.current && gameAudio.current !== next) pauseAudioTrack(gameAudio.current, true);\n    gameAudio.current = next;\n    gameAudio.field = field;\n    if (next === gameAudio.green) { try { next.currentTime = 0; } catch (_) {} }\n    playAudioTrack(next);\n  }\n\n  function startGameAudio() {\n    initGameAudio();\n    gameAudio.active = true;\n    gameAudio.field = '';\n    switchFieldAudio('green');\n  }\n\n  function updateGameAudio() {\n    if (!gameAudio.active) return;\n    switchFieldAudio(state.carY >= CORN_END ? 'dry' : 'green');\n    if (gameAudio.current === gameAudio.green && gameAudio.green.currentTime >= GREEN_FIELD_LOOP_END) {\n      try { gameAudio.green.currentTime = 0; } catch (_) {}\n      playAudioTrack(gameAudio.green);\n    }\n    const brakingNow = state.braking && brakesUnlocked() && state.speed > 4 && (state.mode === 'intro' || state.mode === 'play');\n    if (brakingNow) {\n      if (gameAudio.current) gameAudio.current.volume = 0.32;\n      playAudioTrack(gameAudio.brakes);\n    } else {\n      if (gameAudio.current) gameAudio.current.volume = 0.54;\n      pauseAudioTrack(gameAudio.brakes, true);\n    }\n  }\n\n  function stopGameAudio() {\n    if (!gameAudio.green) return;\n    gameAudio.active = false;\n    gameAudio.field = '';\n    pauseAudioTrack(gameAudio.green, true);\n    pauseAudioTrack(gameAudio.dry, true);\n    pauseAudioTrack(gameAudio.brakes, true);\n    gameAudio.current = null;\n  }\n\n  function trackGameEvent(name, params = {}) {\n    if (typeof window.gtag === 'function') {\n      window.gtag('event', name, params);\n    }\n  }\n\n  function buildGameAnalyticsPayload(result) {\n    const s = typeof scoreValue === 'function'\n      ? scoreValue()\n      : { align: Math.round(alignmentScore() * 100), stopGap: Math.max(0, Math.round(WATER_START - state.carY)), time: Math.max(0, state.gameTimer), score: 0 };\n    return {\n      result,\n      score: s.score,\n      alignment: s.align,\n      cliff_gap: s.stopGap,\n      time_left: Number(s.time.toFixed(1)),\n      distance_travelled: Math.round(state.carY)\n    };\n  }\n\n  function rr(x, y, w, h, r, fillStyle) {\n    ctx.beginPath();\n    ctx.roundRect(Math.round(x), Math.round(y), Math.round(w), Math.round(h), r);\n    ctx.fillStyle = fillStyle;\n    ctx.fill();\n  }\n\n  function rrStroke(x, y, w, h, r, strokeStyle, line = 1) {\n    ctx.beginPath();\n    ctx.roundRect(Math.round(x) + 0.5, Math.round(y) + 0.5, Math.round(w), Math.round(h), r);\n    ctx.strokeStyle = strokeStyle;\n    ctx.lineWidth = line;\n    ctx.stroke();\n  }\n\n  const CAR_SCREEN_Y"
  );

  code = code.replace(
    "    state.transitionStartedAt = now;\n    input.pointers.clear();",
    "    state.transitionStartedAt = now;\n    trackGameEvent('game_start', { event_category: 'game' });\n    startGameAudio();\n    input.pointers.clear();"
  );

  code = code.replace(
    "        state.result = 'TRUCK STOPPED BEFORE THE SHORELINE';",
    "        state.result = 'TRUCK STOPPED BEFORE THE CLIFF';\n        trackGameEvent('game_finish', buildGameAnalyticsPayload('win'));\n        stopGameAudio();"
  );

  code = code.replace(
    "        state.result = 'STOPPED IN TIME, BUT DRONE ALIGNMENT WAS OFF';",
    "        state.result = 'STOPPED IN TIME, BUT DRONE ALIGNMENT WAS OFF';\n        trackGameEvent('game_finish', buildGameAnalyticsPayload('fail_alignment'));\n        stopGameAudio();"
  );

  code = code.replace(
    "        state.result = 'STOPPED TOO EARLY - THE CLIFF IS STILL AHEAD';",
    "        state.result = 'STOPPED TOO EARLY - THE CLIFF IS STILL AHEAD';\n        trackGameEvent('game_finish', buildGameAnalyticsPayload('fail_early_stop'));\n        stopGameAudio();"
  );

  code = code.replace(
    "        state.result = 'NO BRAKE - THE TRUCK DROVE OFF THE CLIFF';",
    "        state.result = 'NO BRAKE - THE TRUCK DROVE OFF THE CLIFF';\n        trackGameEvent('game_finish', buildGameAnalyticsPayload('fail_cliff'));\n        stopGameAudio();"
  );

  code = code.replace(
    "        state.result = 'TIME EXPIRED BEFORE THE TRUCK REACHED THE CLIFF';",
    "        state.result = 'TIME EXPIRED BEFORE THE TRUCK REACHED THE CLIFF';\n        trackGameEvent('game_finish', buildGameAnalyticsPayload('fail_timeout'));\n        stopGameAudio();"
  );

  code = code.replace(
    "    addDust(dt);\n  }",
    "    updateGameAudio();\n    addDust(dt);\n  }"
  );

  code = code.replace(
    "    state.mode = 'menu';\n    state.carX = 112;",
    "    stopGameAudio();\n    state.mode = 'menu';\n    state.carX = 112;"
  );

  code = code.replace(
    /function drawMenuBackground\(\) \{[\s\S]*?\n  \}\n\n  function drawButton/,
    "function drawMenuBackground() {\n    rect(0, 0, W, H, '#000');\n    if (menuBgReady) {\n      const scale = Math.max(W / menuBg.width, H / menuBg.height);\n      const dw = menuBg.width * scale;\n      const dh = menuBg.height * scale;\n      ctx.imageSmoothingEnabled = true;\n      ctx.drawImage(menuBg, (W - dw) / 2, (H - dh) / 2, dw, dh);\n    } else {\n      const g0 = ctx.createLinearGradient(0, 0, 0, H);\n      g0.addColorStop(0, '#050609');\n      g0.addColorStop(0.52, '#162019');\n      g0.addColorStop(1, '#050505');\n      ctx.fillStyle = g0;\n      ctx.fillRect(0, 0, W, H);\n    }\n    const menuBorder = 7;\n    rect(0, 0, W, menuBorder, '#020b17');\n    rect(0, H - menuBorder, W, menuBorder, '#020b17');\n    rect(0, 0, menuBorder, H, '#020b17');\n    rect(W - menuBorder, 0, menuBorder, H, '#020b17');\n  }\n\n  function drawButton"
  );

  code = code.replace(
    /function drawButton\(b, label, primary = false\) \{[\s\S]*?\n  \}\n\n  function drawPosterTitle/,
    "function drawButton(b, label, primary = false) {\n    if (!primary) {\n      ctx.textAlign = 'center';\n      ctx.fillStyle = '#f7f0df';\n      ctx.shadowColor = 'rgba(0,0,0,0.85)';\n      ctx.shadowBlur = 8;\n      ctx.font = '700 16px Montserrat, Arial, sans-serif';\n      ctx.fillText(label, b.x + b.w / 2, b.y + b.h / 2 + 6);\n      ctx.shadowBlur = 0;\n      ctx.textAlign = 'left';\n      return;\n    }\n    drawPremiumPlayButton(b);\n  }\n\n  function drawPosterTitle"
  );

  code = code.replace(
    /function drawPosterTitle\(\) \{[\s\S]*?\n  \}\n\n  function drawMenu/,
    "function drawPosterTitle() {\n    ctx.textAlign = 'center';\n    ctx.shadowColor = 'rgba(0,0,0,0.72)';\n    ctx.shadowBlur = 10;\n    ctx.fillStyle = '#fff6df';\n    ctx.font = '800 34px Montserrat, Arial, sans-serif';\n    ctx.fillText('CORNFIELD', W / 2, 86);\n    ctx.font = '800 36px Montserrat, Arial, sans-serif';\n    ctx.fillText('CHASE', W / 2, 126);\n    ctx.shadowBlur = 0;\n    rect(102, 146, 246, 2, 'rgba(246,237,214,0.72)');\n    ctx.fillStyle = '#ead9b6';\n    ctx.font = '500 13px Montserrat, Arial, sans-serif';\n    ctx.fillText('We are still pioneers', W / 2, 172);\n    ctx.textAlign = 'left';\n  }\n\n  function drawPremiumPlayButton(b) {\n    const t = performance.now() * 0.0024;\n    ctx.save();\n    ctx.shadowColor = 'rgba(0,0,0,0.42)';\n    ctx.shadowBlur = 22;\n    ctx.shadowOffsetY = 8;\n    const bg = ctx.createLinearGradient(b.x, b.y, b.x, b.y + b.h);\n    bg.addColorStop(0, '#202127');\n    bg.addColorStop(0.48, '#0d0d12');\n    bg.addColorStop(1, '#202126');\n    rr(b.x, b.y, b.w, b.h, 24, bg);\n    ctx.restore();\n    rrStroke(b.x, b.y, b.w, b.h, 24, 'rgba(255,255,255,0.08)', 1);\n\n    const ix = b.x + 13;\n    const iy = b.y + 9;\n    const isz = b.h - 18;\n    const cx = ix + isz / 2;\n    const cy = iy + isz / 2;\n\n    ctx.save();\n    ctx.lineWidth = 5;\n    ctx.lineCap = 'round';\n    const colors = ['#ff4b63', '#ffb12d', '#fff278', '#28e5ff', '#4948ff', '#ff48d2'];\n    for (let i = 0; i < 6; i++) {\n      ctx.beginPath();\n      ctx.strokeStyle = colors[i];\n      ctx.globalAlpha = 0.86;\n      ctx.arc(cx, cy, isz / 2 + 5, t + i * 1.04, t + i * 1.04 + 0.58);\n      ctx.stroke();\n    }\n    ctx.restore();\n\n    const iconBg = ctx.createLinearGradient(ix, iy, ix + isz, iy + isz);\n    iconBg.addColorStop(0, '#ffffff');\n    iconBg.addColorStop(1, '#f4f4f7');\n    rr(ix, iy, isz, isz, 14, iconBg);\n    ctx.fillStyle = '#101014';\n    ctx.font = '800 29px Montserrat, Arial, sans-serif';\n    ctx.textAlign = 'center';\n    ctx.fillText('↗', ix + isz / 2, iy + isz / 2 + 10);\n\n    ctx.fillStyle = '#ffffff';\n    ctx.font = '700 26px Montserrat, Arial, sans-serif';\n    ctx.textAlign = 'left';\n    ctx.fillText('PLAY', ix + isz + 24, b.y + b.h / 2 + 10);\n    ctx.textAlign = 'left';\n  }\n\n  function drawMenu"
  );

  code = code.replace(
    /function drawHowTo\(\) \{[\s\S]*?\n  \}\n\n  function scoreValue/,
    "function drawHowTo() {\n    drawMenuBackground();\n    const panelX = 28;\n    const panelY = 64;\n    const panelW = W - 56;\n    const panelH = 660;\n    rr(panelX, panelY, panelW, panelH, 32, '#9b86ff');\n\n    ctx.textAlign = 'center';\n    ctx.fillStyle = '#ffffff';\n    ctx.font = '800 25px Montserrat, Arial, sans-serif';\n    ctx.fillText('HOW TO PLAY', W / 2, panelY + 48);\n    ctx.font = '500 12px Montserrat, Arial, sans-serif';\n    ctx.fillStyle = 'rgba(255,255,255,0.78)';\n    ctx.fillText('follow the drone and stop at the cliff', W / 2, panelY + 72);\n\n    function avatar(x, y, bg, type) {\n      rr(x - 23, y - 23, 46, 46, 23, bg);\n      if (type === 'truck') drawTruckAt(x, y + 10, 0.48, 0);\n      if (type === 'drone') drawDroneAt(x, y + 2, 0.32, false);\n      if (type === 'trail') drawTrailIcon(x, y + 2);\n      if (type === 'cliff') drawShoreIcon(x, y);\n    }\n\n    function row(y, title, body, type, active = false, tag = '') {\n      const x = panelX + 18;\n      const w = panelW - 36;\n      const h = 82;\n      rr(x, y, w, h, 26, active ? '#080806' : '#ffffff');\n      ctx.textAlign = 'left';\n      avatar(x + 34, y + h / 2, active ? '#d7d0ff' : '#f2f0ff', type);\n      ctx.fillStyle = active ? '#ffffff' : '#8b86a0';\n      ctx.font = '500 13px Montserrat, Arial, sans-serif';\n      ctx.fillText(title, x + 76, y + 30);\n      ctx.fillStyle = active ? '#ffffff' : '#141414';\n      ctx.font = '600 13px Montserrat, Arial, sans-serif';\n      ctx.fillText(body, x + 76, y + 55);\n      if (tag) {\n        ctx.textAlign = 'right';\n        ctx.fillStyle = '#f2b850';\n        ctx.font = '700 11px Montserrat, Arial, sans-serif';\n        ctx.fillText(tag, x + w - 18, y + 50);\n      }\n    }\n\n    row(panelY + 100, 'Truck', 'Steer left and right through the cornfield.', 'truck');\n    row(panelY + 196, 'Drone', 'Keep the truck aligned with the drone.', 'drone');\n    row(panelY + 292, 'Trail', 'Every move damages and marks the field.', 'trail');\n    row(panelY + 388, 'You', 'Brake after dry field, before the cliff.', 'cliff', true, 'Goal');\n    row(panelY + 484, 'HUD', 'Watch CLIFF, TIME, and ALIGNMENT.', 'drone');\n\n    drawButton(buttons.howPlay, 'PLAY', true);\n    ctx.textAlign = 'center';\n    ctx.fillStyle = '#ffffff';\n    ctx.font = '700 16px Montserrat, Arial, sans-serif';\n    ctx.fillText('CLOSE', buttons.howClose.x + buttons.howClose.w / 2, buttons.howClose.y + 27);\n    ctx.textAlign = 'left';\n  }\n\n  function scoreValue"
  );

  code = code.replace(
    /function drawResult\(\) \{[\s\S]*?\n  \}\n\n  function wrapText/,
    "function drawResult() {\n    drawWorld();\n    drawTrail();\n    drawDust();\n    drawDrone();\n    drawTruck();\n    drawHud();\n    const s = scoreValue();\n    rr(48, 182, W - 96, 402, 28, 'rgba(6,7,12,0.92)');\n    rrStroke(48, 182, W - 96, 402, 28, state.mode === 'win' ? '#d6af61' : '#a95a3d', 2);\n    ctx.textAlign = 'center';\n    ctx.fillStyle = state.mode === 'win' ? '#f6e081' : '#ef986e';\n    ctx.font = '800 22px Montserrat, Arial, sans-serif';\n    ctx.fillText(state.mode === 'win' ? 'MISSION COMPLETE' : 'CHASE FAILED', W / 2, 240);\n    ctx.fillStyle = '#fff6df';\n    ctx.font = '800 46px Montserrat, Arial, sans-serif';\n    ctx.fillText(String(s.score), W / 2, 310);\n    ctx.fillStyle = '#b9b2a5';\n    ctx.font = '600 12px Montserrat, Arial, sans-serif';\n    ctx.fillText('SCORE', W / 2, 333);\n\n    function metric(y, label, value) {\n      rr(78, y, W - 156, 42, 18, 'rgba(255,255,255,0.08)');\n      ctx.textAlign = 'left';\n      ctx.fillStyle = '#d8d0bd';\n      ctx.font = '700 12px Montserrat, Arial, sans-serif';\n      ctx.fillText(label, 98, y + 27);\n      ctx.textAlign = 'right';\n      ctx.fillStyle = '#ffffff';\n      ctx.fillText(value, W - 98, y + 27);\n    }\n\n    metric(360, 'ALIGNMENT', s.align + '%');\n    metric(410, 'CLIFF GAP', s.stopGap + 'm');\n    metric(460, 'TIME LEFT', s.time.toFixed(1) + 's');\n    ctx.textAlign = 'center';\n    ctx.fillStyle = '#cdbb95';\n    ctx.font = '600 12px Montserrat, Arial, sans-serif';\n    wrapText(state.result, 36).forEach((line, i) => ctx.fillText(line, W / 2, 528 + i * 18));\n    drawButton({ x: 112, y: 604, w: 226, h: 56 }, 'MAIN MENU', true);\n    ctx.textAlign = 'left';\n  }\n\n  function wrapText"
  );

  code = code.replaceAll('SHORELINE', 'CLIFF').replaceAll('shoreline', 'cliff');

  (0, eval)(code + '\n//# sourceURL=fieldchase-runtime.js');
})();